...
def search(image_query, min_range, max_range):
    ...
    for item in data['data']:
        if item['can_compare'] == True:
            ...
            urlforlink = "https://price-api.datayuge.com/api/v1/compare/detail?api_key=" + api + "&id="+ item["product_id"]
            newjson_obj = urllib.request.urlopen(urlforlink)
            newdata = json.load(newjson_obj)
            i = 0
            for newitem in newdata["data"]["stores"]:
                store = ["amazon", "flipkart", "snapdeal", "ebay", "paytm", "croma", "yebhi", "indiatimes", "homeshop18", "naaptol", "infibeam", "tatacliq", "shopclues"]
